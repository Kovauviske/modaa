...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		{
			hintSilent format["Tempo restante:\n %1\n\nPode pagar fiança: %3\nTempo para Fiança: $%2",_countDown,[life_bail_amount] call life_fnc_numberText];
		}
		else
		{
			hintSilent format["Tempo restante:\n %1\n",_countDown];
...
switch (true) do
{
	...
	case (_esc) :
	{
		...
		[[0,format["%1 Você escapou da cadeia",profileName]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
