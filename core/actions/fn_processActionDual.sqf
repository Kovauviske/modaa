...
if ((_type == "uranium2") && (life_inv_puranium == 0)) exitWith {hint "Sie haben kein Produkt um das Uran zu l?sen.";};

//unprocessed item,processed item, cost if no license,Text to display (I.e Processing (percent) ..."
_itemInfo = switch (_type) do
{
    case "uranium2": {["uranium2","puranium","uranium3",6000,"Urânio Legal"];};
    case "uranium2b": {["uranium2","ipuranium","uranium3",6000,"Urânio Ilegal"];};
    default {[]};
};
...
if(_hasLicense) then
{
    ...
    if(player distance _vendor > 10) exitWith {hint "Você precisa ficar dentro de 10m para o Processo."; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    else
{
    ...
    if(life_cash < _cost) exitWith {hint format["Você precisa $%1 para processar sem a licença",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
