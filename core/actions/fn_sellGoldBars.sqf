...
if(life_inv_goldbar == 0) exitWith {
	titleText["Você não tem qualquer barra(s) de ouro para vender.","PLAIN"];
};
...
if([false,"goldbar",life_inv_goldbar] call life_fnc_handleInv) then {
	titleText[format["Você vendeu %1 Barra(s) de Ouro por $%2",_val,[_price] call life_fnc_numberText],"PLAIN"];
