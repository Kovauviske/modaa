    ...
    if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
        ...
        if(count _sPos > 1 && {_distance < 15}) then {
            _text = switch (true) do {
                case ((goggles _x) in _goggles && playerSide in [civilian,west,independent]): {format["<t color='#000000'>Pessoa encapuzada</t>"];};
                case ((headgear _x) in _headgear && playerSide in [civilian,west,independent]): {format["<t color='#000000'>Pessoa encapuzada</t>"];};
                case ((uniform _x) in _uniform && playerSide in [civilian,west,independent]): {format["<t color='#000000'>Pessoa encapuzada</t>"];};
                ...
                case ((!isNil {_x getVariable "name"} && playerSide == independent)): {format["<t color='#FF0000'><img image='a3\ui_f\data\map\MapControl\hospital_ca.paa' size='1.5'></img></t> %1",_x getVariable ["name","Pessoa encapuzada"]]};
                default {
