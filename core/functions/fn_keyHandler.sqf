...
switch (_code) do
{
	...
	case 33:
	{
		if (vehicle player != player && (typeOf vehicle player) in["C_Offroad_01_repair_F","B_Truck_01_transport_F"]) exitWith {hint "Siren indisponíveis"};
	...
	case 24:
	{
		...
		if !(license_civ_rebel) exitWith { hint "Você precisa de uma licença para rebeldes para prender pessoas"; };
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,east]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			...
			{
				[] call life_fnc_restrainAction;
				hint "Você prendeu ele! Use o menu de interação para mais opções ";
			} else {
				hint "Você não tem algema";
			};
		};
	};
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						systemChat "Você trancou seu veículo.";
	...
	case 26:
	{
		if(vehicle player != player) exitWith {hint "Você não pode usar Faixa de prego"};
		if(playerSide == west) then {

		if(!isNull life_spikestrip) exitWith {hint "Você já colocou a faixa de prego"};
	...
	case 59: // F1
	{
		if(__GETC__(life_adminlevel) > 1) exitWith {hint "O ADM Está on-line"};
		closeDialog 0;
