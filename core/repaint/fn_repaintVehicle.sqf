...
if(isNil {_vehicleData}) exitWith {hint "Carro com defeito?.";_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
if ((getPlayerUID player) != _vehOwner) exitWith {hint "Você não é o proprietário!";_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
		...
		if(player != vehicle player) exitWith {titleText["Você tem que sair do carro para  ser pintado","PLAIN"];_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
